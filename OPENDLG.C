#pragma	title("Icon/Pointer Viewer  --  Version 1  --  (OpenDlg.C)")
...
if ( !(ulErr = DosFindFirst("*.*", &hDir, FILE_NORMAL, (PVOID)pfindbuf3, 71UL *	sizeof(FILEFINDBUF3),
   ...
   do
	   ...
	   if (	((pfindbuf->attrFile & FILE_DIRECTORY) != FILE_DIRECTORY) &&
		(pch = strrchr(pfindbuf->achName, '.'))	!= NULL	)
	       if ( !memicmp(pch, ".ptr", 5) ||	!memicmp(pch, ".ico", 5) || !memicmp(pch, ".cur", 5) )
	       {
	       ++cFiles;
